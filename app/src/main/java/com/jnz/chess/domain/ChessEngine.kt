...
object ChessEngine {
    ...
    fun fenToState(fen: String): FenParseResult {
        try {
            ...
            return FenParseResult.Success(state.copyWith(
    ...
    /** Generate legal moves for a single piece at (row, col) — avoids generating all moves. */
    fun getLegalMovesForSquare(state: GameState, row: Int, col: Int, color: Color? = null): List<Move> {
        val c = color ?: state.currentTurn
        val piece = state.board[row][col] ?: return emptyList()
        if (piece.color != c) return emptyList()
        val moves = mutableListOf<Move>()
        when (piece.type) {
            PieceType.PAWN -> generatePawnMoves(state, row, col, piece, moves)
            PieceType.KNIGHT -> generateKnightMoves(state, row, col, piece, moves)
            PieceType.BISHOP -> generateSlidingMoves(state, row, col, piece, bishopDirs, moves)
            PieceType.ROOK -> generateSlidingMoves(state, row, col, piece, rookDirs, moves)
            PieceType.QUEEN -> generateSlidingMoves(state, row, col, piece, queenDirs, moves)
            PieceType.KING -> generateKingMoves(state, row, col, piece, moves)
        }
        return moves.filter { isLegal(state, it, c) }
    }

    fun makeMove(state: GameState, move: Move): GameState {
        val piece = state.board[move.fromRow][move.fromCol] ?: return state
        val newBoard = Array(8) { state.board[it].copyOf() }
        ...
        val newCounts = newState.positionCounts.toMutableMap()
        newCounts[posKey] = (newCounts[posKey] ?: 0) + 1
        val repeated = newCounts[posKey] ?: 1
        val newStateWithHistory = newState.copyWith(
            newPositionHistory = updatedPositionHistory,
            newPositionCounts = newCounts
        )
    ...
    fun isLegal(state: GameState, move: Move, color: Color): Boolean {
        ...
        // Copy the board to avoid mutating GameState's internal array
        val board = Array(8) { state.board[it].copyOf() }

        val fromRow = move.fromRow; val fromCol = move.fromCol
        val toRow = move.toRow; val toCol = move.toCol
        ...
        val kingPos = if (piece.type == PieceType.KING) (toRow to toCol)
            else findKingInBoard(board, color)
        return kingPos != null && !isSquareAttackedInBoard(board, kingPos.first, kingPos.second, oppColor)
    }
