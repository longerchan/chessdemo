...
class JsonOpeningBookProvider(private val context: Context, private val fileName: String = "opening_book.json") :
    ...
    private fun moveToUci(move: com.jnz.chess.domain.Move): String {
        val files = "abcdefgh"
        ...
        val promo = if ((toRank == 1 || toRank == 8) && move.promotionType != null) {
            when (move.promotionType) {
                com.jnz.chess.domain.PieceType.QUEEN -> "q"
                com.jnz.chess.domain.PieceType.ROOK -> "r"
                com.jnz.chess.domain.PieceType.BISHOP -> "b"
                com.jnz.chess.domain.PieceType.KNIGHT -> "n"
                else -> "q"
            }
        } else ""
