...
object PolyglotBook {
    ...
    fun lookup(state: com.jnz.chess.domain.GameState): List<Pair<String, Int>> {
        return lookupByState(state)
    }

    /** Look up moves for a GameState using FEN from StockfishAI. */
    fun lookupByState(state: com.jnz.chess.domain.GameState): List<Pair<String, Int>> {
        if (!loaded) return emptyList()
        val fen = com.jnz.chess.ai.StockfishAI.stateToFenPublic(state)
